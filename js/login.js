...
for(let x = 0; x < listaDeUsuarios.length; x++) {
    if{usuarioLogado.nomeDeUsuario == listaDeUsuarios[x].nomeUsuario &&
    usuarioLogado.senhaUsuariuLogado
        nomeCompleto: ""
    }
}
...
const usuarioLogado = {
    ...
    senhaUsuariuLogado: inputSenha.value
}
...
addEventListener("click", (evt)=>{
    ...
    if(evt.target.id == "btnSubmit"){
        ...
        listaDeUsuarios.forEach( (usuario)=>{
            ...
            if(usuarioLogado.nomeUsuarioLogado == "achila" && usuarioLogado.
            senhaUsuarioLogado == "123456"){
                alert("VALIDADO")
            }else{
                alert("NÃO VALIDADO")
            }
